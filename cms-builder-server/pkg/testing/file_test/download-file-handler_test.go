...
func UploadMockFile(t *testing.T, testBed *TestUtils, content string, fileName string) *models.File {
	...
	// Execute the handler
	rr := ExecuteHandler(t, CreateStoredFilesHandler(testBed.Db, testBed.Store, "http://localhost:8080")(testBed.Src, testBed.Db), req)
...
func TestDownloadStoredFileHandler_Success(t *testing.T) {
	...
	req := CreateTestRequest(t, http.MethodGet, path, "", true, testBed.AdminUser, testBed.Logger)
	...
	router.HandleFunc(route, DownloadStoredFileHandler(testBed.Mgr, testBed.Db, testBed.Store))
...
func TestDownloadStoredFileHandler_InvalidMethod(t *testing.T) {
	...
	// Execute the handler
	rr := httptest.NewRecorder()
	handler := DownloadStoredFileHandler(testBed.Mgr, testBed.Db, testBed.Store)
...
func TestDownloadStoredFileHandler_PermissionDenied(t *testing.T) {
	...
	req := CreateTestRequest(t, http.MethodGet, "/files/"+file.StringID(), "", true, testBed.NoRoleUser, testBed.Logger)
...
func TestDownloadStoredFileHandler_FileNotFound(t *testing.T) {
	...
	req := CreateTestRequest(t, http.MethodGet, "/files/99999/download", "", true, testBed.AdminUser, testBed.Logger)
...
func TestDownloadStoredFileHandler_FileOpenError(t *testing.T) {
	...
	// Create a test request with the GET method
	req := CreateTestRequest(t, http.MethodGet, "/files/"+file.StringID()+"/download", "", true, testBed.AdminUser, testBed.Logger)
