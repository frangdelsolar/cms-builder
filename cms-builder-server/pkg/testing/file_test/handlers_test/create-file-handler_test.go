...
func TestCreateStoredFilesHandler_EmptyFile(t *testing.T) {
	...
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)
	_, err := writer.CreateFormFile("file", "emptyfile.png")
	assert.NoError(t, err)
	...
	// Assertions
	assert.Equal(t, http.StatusBadRequest, rr.Code) // Expect 400 Bad Request
	assert.Contains(t, rr.Body.String(), "file is empty")
}
...
func TestCreateStoredFilesHandler_LargeFile(t *testing.T) {
	...
	// Assertions
	assert.Equal(t, http.StatusBadRequest, rr.Code) // Expect 400 Bad Request
	assert.Contains(t, rr.Body.String(), "file is too large")
}
...
func TestCreateStoredFilesHandler_InvalidMethod(t *testing.T) {
	...
	req := testPkg.CreateTestRequest(t, http.MethodGet, "/files", "", true, testBed.AdminUser, testBed.Logger)
...
func TestCreateStoredFilesHandler_PermissionDenied(t *testing.T) {
	...
	req := testPkg.CreateTestRequest(t, http.MethodPost, "/files", body.String(), true, testBed.NoRoleUser, testBed.Logger)
	req.Header.Set("Content-Type", writer.FormDataContentType())
