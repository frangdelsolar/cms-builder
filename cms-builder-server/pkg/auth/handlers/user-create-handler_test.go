...
func TestUserCreateHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			path:           "/user/new",
			requestBody:    `{"firstName": "` + testPkg.RandomString(10) + `", "lastName": "` + testPkg.RandomString(10) + `", "email": "` + testPkg.RandomEmail() + `"}`,
			user:           bed.AdminUser,
...

		{
			...
			method:         http.MethodPost,
			path:           "/user/new",
			requestBody:    `{"firstName": "` + testPkg.RandomString(10) + `", "lastName": "` + testPkg.RandomString(10) + `"}`,
			user:           bed.AdminUser,
			expectedStatus: http.StatusBadRequest,
