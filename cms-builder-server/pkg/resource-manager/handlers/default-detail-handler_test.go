...
func TestDefaultDetailHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			user:        bed.AdminUser,
			setup: func() string {
				instance := testPkg.CreateMockResourceInstance(bed.VisitorUser.ID) // Resource owned by another user
				bed.Db.DB.Create(&instance)
				return "/mock-struct/" + instance.StringID()
			},
			expectedStatus: http.StatusOK,
		...
		},
		{
			...
			path:        "/mock-struct/123",
			requestBody: "",
			user:        testPkg.CreateAllAllowedUser(),
			setup: func() string {
				instance := testPkg.CreateMockResourceInstance(bed.AdminUser.ID) // Resource owned by another user
				bed.Db.DB.Create(&instance)
				return "/mock-struct/" + instance.StringID()
			},
			expectedStatus: http.StatusNotFound,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router.HandleFunc("/mock-struct/{id}", rmHandlers.DefaultDetailHandler(bed.Src, bed.Db))
